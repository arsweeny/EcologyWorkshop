  ...
  mutate(temperature_celsius = ifelse(season == "spring", rnorm(n, mean = 12, sd = 3),
                      ifelse(season == "summer", rnorm(n, mean = 25, sd = 5),
                             ifelse(season == "fall", rnorm(n, mean = 15, sd = 4),
                                    rnorm(n, mean = 5, sd = 5))))) %>% 
  mutate(temperature_celsius = ifelse(temperature_celsius < 0, 0, 
                                      temperature_celsius))
data  <- data %>% 
...
# some modifications that would be errors ---------------------------------
data_with_errors <- data

# Introduce random measurement/data entry errors for temperature
set.seed(123)  # For reproducibility
num_temp_errors <- round(0.01 * nrow(data_with_errors))  # 5% of rows with errors

# Select random rows to introduce errors
error_indices <- sample(1:nrow(data_with_errors), size = num_temp_errors, replace = FALSE)

# Introduce implausible values for temperature
# Some extremely high values and some extremely low (e.g., -100 or 100)
data_with_errors$temperature[error_indices] <- sample(c(70, 100), size = num_temp_errors, replace = TRUE)

# View a summary to check the changes
summary(data_with_errors$temperature)
...
data_with_errors <- data_with_errors %>% 
...
# Save dataset
write.csv(data, "Data/wild_disease_dataset_with_issues.csv", row.names = FALSE)
